...
import com.example.xiaojin20135.esptouch.EsptouchResult;
import com.example.xiaojin20135.esptouch.IEsptouchListener;
import com.example.xiaojin20135.esptouch.IEsptouchResult;
import com.example.xiaojin20135.esptouch.protocol.EsptouchGenerator;
import com.example.xiaojin20135.esptouch.udp.UDPSocketClient;
import com.example.xiaojin20135.esptouch.udp.UDPSocketServer;
import com.example.xiaojin20135.esptouch.util.ByteUtil;
import com.example.xiaojin20135.esptouch.util.EspNetUtil;
...
public class __EsptouchTask implements __IEsptouchTask {
	...
	private void __listenAsyn(final int expectDataLen) {
		mTask = new Thread() {
			public void run() {
                ...
                Log.d(TAG,"apSsidAndPassword = " + ByteUtil.bytsToStr(apSsidAndPassword));
				...
				while (mEsptouchResultList.size() < mParameter.getExpectTaskResultCount() && !mIsInterrupt) {
					...
					if (receiveBytes != null) {
                        ...
                        Log.d(TAG,"receiveBytes = " + ByteUtil.bytsToStr(receiveBytes));
					} else {
						receiveOneByte = -1;
					}
	...
	private boolean __execute(IEsptouchGenerator generator) {
		Log.d(TAG,"in __execute");
		long startTime = System.currentTimeMillis();
		long currentTime = startTime;
		long lastTime = currentTime - mParameter.getTimeoutTotalCodeMillisecond();
		byte[][] gcBytes2 = generator.getGCBytes2();   //guide code
		byte[][] dcBytes2 = generator.getDCBytes2();   // data code
		//打印日志
		ByteUtil.bytesArrToStr("gcBytes2",gcBytes2);
		ByteUtil.bytesArrToStr("dcBytes2",dcBytes2);
		int index = 0;
		while (!mIsInterrupt) {
			if (currentTime - lastTime >= mParameter.getTimeoutTotalCodeMillisecond()) {
				...
				while (!mIsInterrupt && System.currentTimeMillis() - currentTime < mParameter.getTimeoutGuideCodeMillisecond()) {
					mSocketClient.sendData(gcBytes2, mParameter.getTargetHostname(), mParameter.getTargetPort(), mParameter.getIntervalGuideCodeMillisecond());
				...
				mSocketClient.sendData(dcBytes2, index, ONE_DATA_LEN, mParameter.getTargetHostname(), mParameter.getTargetPort(), mParameter.getIntervalDataCodeMillisecond());
