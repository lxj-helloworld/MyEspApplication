...
public class ByteUtil {
	 ...
	 * @param arrs
	 */
	public static void bytesArrToStr(String name,byte[][] arrs){
		if(arrs != null){
			int len = arrs.length;
			for(int i=0;i<len;i++){
				byte[] tempArr = arrs[i];
				...
				for(int j=0;j<tempArr.length;j++){
					tempStr.append(Integer.toHexString(tempArr[j]) + " ");
				}
				Log.d(TAG,name+"["+ i + "]"+tempStr.toString());
			}
		}else{
		}
	}
	...
	public static String bytsToStr(byte[] bytes){
